...
namespace Copa2014.Negocio
{
    class Fases
    {
        ...
        public void salvar(out string mensagem)
        {
            ...
            if (_codigo == 0)
            {
                banco.SqlCmd.CommandText = "insert into fases (fas_descricao) values (@descricao)";
                banco.SqlCmd.Parameters.AddWithValue("@descricao", _descricao);
            }
            else
            {
                banco.SqlCmd.CommandText = "update fases set fas_descricao = @descricao where fas_codigo = @codigo";
                banco.SqlCmd.Parameters.AddWithValue("@descricao", _descricao);
        ...
        public void excluir(out string mensagem)
        {
            ...
            banco.SqlCmd.CommandText = "delete from fases where fas_codigo = @codigo";
            banco.SqlCmd.Parameters.AddWithValue("@codigo", _codigo);
            if (!banco.executarComando())
        ...
        {
            banco.abrirConexao();
            banco.SqlCmd.CommandText = "select * from fases where fas_codigo = @codigo";
            banco.SqlCmd.Parameters.AddWithValue("@codigo", _codigo);
            ...
            if (dt.Rows.Count > 0)
            {
                _codigo = Convert.ToInt32(dt.Rows[0]["fas_codigo"]);
                _descricao = Convert.ToString(dt.Rows[0]["fas_descricao"]);
                return true;
            }
            else
                return false;
